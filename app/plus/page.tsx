...
export default async function PlusPage() {
  ...
  if (!supabase) {
    return <PlusDashboard authenticated={false} premium={false} serverRuns={[]} manageUrl={null} />;
  }
  ...
  const [entitlementResult, runsResult] = await Promise.all([
    supabase
      .from('entitlements')
      .select('status,provider,provider_payload')
  ...
  const premium = entitlementIsActive(entitlementResult.data);
  const manageUrl = entitlementManageUrl(entitlementResult.data);
