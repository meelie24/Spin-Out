...
export async function POST(request: Request) {
  ...
  if (!body?.profile || !body.record || !validUuid(body.record.id)) {
    return NextResponse.json({ synced: false }, { status: 400 });
  }

  const serialized = JSON.stringify(body);
  if (serialized.length > 150_000) {
    return NextResponse.json({ synced: false }, { status: 413 });
  }
  ...
  const profileWrite = supabase.from('user_profiles').upsert({
    user_id: auth.user.id,
    profile: body.profile,
    updated_at: new Date().toISOString(),
  }, { onConflict: 'user_id' });

  const runWrite = supabase.from('run_history').upsert({
    id: body.record.id,
    user_id: auth.user.id,
  ...
  const [profileResult, runResult] = await Promise.all([profileWrite, runWrite]);
